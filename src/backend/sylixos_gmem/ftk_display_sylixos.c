...
#define fb_size(fb)     ((fb)->varinfo.GMVI_ulXRes * (fb)->varinfo.GMVI_ulYRes * \
                        fb->varinfo.GMVI_ulBitsPerPixel / 8)
...
FtkDisplay* ftk_display_sylixos_create(const char* filename)
{
    ...
    if(gmem_open(fb, filename) == 0)
    {
        ...
        int bits_per_pixel = fb->varinfo.GMVI_ulBitsPerPixel;
        ...
        thiz = ftk_display_mem_create(format, fb->varinfo.GMVI_ulXRes, fb->varinfo.GMVI_ulYRes,
            fb->bits, gmem_close, fb);
