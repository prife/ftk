...
static int lua_ftk_display_reg_update_listener(lua_State* L)
{
	tolua_Error err = {0};
	Ret retv;
	FtkDisplay* thiz;
	char* on_update;
	int param_ok = tolua_isusertype(L, 1, "FtkDisplay", 0, &err) && tolua_isstring(L, 2, 0, &err);
...
static int lua_ftk_display_unreg_update_listener(lua_State* L)
{
	tolua_Error err = {0};
	Ret retv;
	FtkDisplay* thiz;
...
static int lua_ftk_display_notify(lua_State* L)
{
	tolua_Error err = {0};
	Ret retv;
	FtkDisplay* thiz;
	...
	int xoffset;
	int yoffset;
	int param_ok = tolua_isusertype(L, 1, "FtkDisplay", 0, &err) && tolua_isnumber(L, 2, 0, &err) && tolua_isusertype(L, 3, "FtkBitmap", 0, &err) && tolua_isusertype(L, 4, "FtkRect", 0, &err) && tolua_isnumber(L, 5, 0, &err) && tolua_isnumber(L, 6, 0, &err);

	return_val_if_fail(param_ok, 0);

	thiz = tolua_tousertype(L, 1, 0);
	before = tolua_tonumber(L, 2, 0);
	bitmap = tolua_tousertype(L, 3, 0);
	rect = tolua_tousertype(L, 4, 0);
	xoffset = tolua_tonumber(L, 5, 0);
	yoffset = tolua_tonumber(L, 6, 0);
	retv = ftk_display_notify(thiz, before, bitmap, rect, xoffset, yoffset);
	tolua_pushnumber(L, (lua_Number)retv);
...
int tolua_ftk_display_init(lua_State* L)
{
	...
	tolua_function(L, "RegUpdateListener", lua_ftk_display_reg_update_listener);
	tolua_function(L, "UnregUpdateListener", lua_ftk_display_unreg_update_listener);
	tolua_function(L, "Notify", lua_ftk_display_notify);
