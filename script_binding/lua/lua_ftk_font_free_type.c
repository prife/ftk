...
#include "lua_ftk_font_free_type.h"
#include "lua_ftk_callbacks.h"

static void tolua_reg_types (lua_State* L)
{
	tolua_usertype(L, "FtkFontFreeType");
}

static int lua_ftk_font_freetype_create(lua_State* L)
{
	tolua_Error err = {0};
	FtkFont* retv;
	char* filename;
...
int tolua_ftk_font_free_type_init(lua_State* L)
{
	...
	tolua_cclass(L,"FtkFontFreeType", "FtkFontFreeType", "FtkFont", NULL);
	tolua_beginmodule(L, "FtkFontFreeType");
	tolua_function(L, "Create", lua_ftk_font_freetype_create);
	tolua_endmodule(L);
	tolua_endmodule(L);
