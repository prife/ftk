...
typedef struct 
{
	...
	/*only_globals used to control code gen of ftk_typedef.idl*/
...
static gboolean lua_code_gen_begin_interface(const char* name, const char* parent, CodeGenInfo *info)
{
	...
	g_string_append_printf(info->str_init, "int tolua_%s_init(lua_State* tolua_S)\n",
	...
	{
		g_string_append_printf(info->str_init,  "	tolua_cclass(tolua_S,\"%s\", \"%s\", \"%s\", NULL);\n",
	...
	fprintf(info->h, "int tolua_%s_init(lua_State* tolua_S);\n", info->interface_lower);
...
static void int_type_init(TypeInfo* info)
{
	...
	strcpy(info->push     , "	tolua_pushnumber(tolua_S, (%s)retv);\n");
	strcpy(info->free     , "");
...
static void str_type_init(TypeInfo* info)
{
	...
	strcpy(info->push     , "	tolua_pushstring(tolua_S, (%s)retv);\n");
	strcpy(info->free     , "");
...
static void str_array_type_init(TypeInfo* info)
{
	...
	strcpy(info->name     , "char**");
	strcpy(info->lua_name , "char**");
	strcpy(info->check    , "tolua_istable(tolua_S, %d, 0, &err)");
...
static void userdata_type_init(const char* name, TypeInfo* info)
{
	...
	strcpy(info->push     , "	tolua_pushusertype(tolua_S, (%s)retv, \"");
